...
public class PulsarAuthorizationProviderXzzh implements AuthorizationProvider {
  public static final String POLICIES = "policies";
  private static final Logger log = LoggerFactory.getLogger(PulsarAuthorizationProviderXzzh.class);
  private static final String POLICY_ROOT = "/admin/policies/";
  private static final String POLICIES_READONLY_FLAG_PATH = "/admin/flags/policies-readonly";
  private ServiceConfiguration conf;
  private PulsarResources pulsarResources;

  public PulsarAuthorizationProviderXzzh() {}

  public PulsarAuthorizationProviderXzzh(
      ServiceConfiguration conf, ConfigurationCacheService configCache) throws IOException {
   ...
   * @param configCache pulsar zk configuration cache service
  ...
  /**
   * Check if the specified role has permission to send messages to the specified fully qualified
   * topic name.<br>
  ...
  /**
   * Check if the specified role has permission to receive messages from the specified fully
   * qualified topic name.<br>
  ...
  public CompletableFuture<Boolean> canConsumeAsync(
    ...
    if (StringUtils.isNotBlank(role)) {
      clientRoles.addAll(Arrays.asList(role.split(",")));
    }
              ...
              policies -> {
                if (policies.isEmpty()) {
                  ...
                  if (isNotBlank(subscription)) {
                    // validate if role is authorize to access subscription. (skip validatation if
                    // authorization
                    // list is empty)
                    Set<String> roles =
                        policies
                    ...
                    if (roles != null && !roles.isEmpty()) {
                      var result = clientRoles.stream().noneMatch(roles::contains);
                      if (result) {
                        log.warn(
                            "[{}] is not authorized to subscribe on {}-{}",
                            role,
                            topicName,
                            subscription);
                        PulsarServerException ex =
                            new PulsarServerException(
                                String.format(
                                    "%s is not authorized to access subscription %s on topic %s",
                    ...
                    if (policies.get().subscription_auth_mode == SubscriptionAuthMode.Prefix) {
                      var result = clientRoles.stream().noneMatch(subscription::startsWith);
                      if (result) {
                        PulsarServerException ex =
                            new PulsarServerException(
                                String.format(
  ...
  /**
   * Check whether the specified role can perform a lookup for the specified topic. <br>
   * 检查指定角色是否可以执行指定主题的查找。
   *
   * <p>For that the caller needs to have producer or consumer permission.
   *
  ...
  public CompletableFuture<Boolean> allowFunctionOpsAsync(
    ...
    return allowTheSpecifiedActionOpsAsync(
        namespaceName, role, authenticationData, AuthAction.functions);
  }
  ...
  public CompletableFuture<Boolean> allowSourceOpsAsync(
      NamespaceName namespaceName, String role, AuthenticationDataSource authenticationData) {
    return allowTheSpecifiedActionOpsAsync(
        namespaceName, role, authenticationData, AuthAction.sources);
  }
  ...
  public CompletableFuture<Boolean> allowSinkOpsAsync(
      NamespaceName namespaceName, String role, AuthenticationDataSource authenticationData) {
    return allowTheSpecifiedActionOpsAsync(
        namespaceName, role, authenticationData, AuthAction.sinks);
  }
  ...
  private CompletableFuture<Boolean> allowTheSpecifiedActionOpsAsync(
    ...
    try {
      pulsarResources
          .getNamespaceResources()
          .getAsync(POLICY_ROOT + namespaceName.toString())
          .thenAccept(
              policies -> {
                if (policies.isEmpty()) {
                  if (log.isDebugEnabled()) {
                  ...
                  for (String cr : clientRoles) {
                    Set<AuthAction> namespaceActions = namespaceRoles.get(cr);
                  ...
                  if (conf.isAuthorizationAllowWildcardsMatching()) {
                    for (String cr : clientRoles) {
                      if (checkWildcardPermission(cr, authAction, namespaceRoles)) {
          ...
          .exceptionally(
              ex -> {
                log.warn(
                    "Client  with Role - {} failed to get permissions for namespace - {}. {}",
                    role,
                    namespaceName,
                    ex.getMessage());
                permissionFuture.completeExceptionally(ex);
  ...
  public CompletableFuture<Void> grantPermissionAsync(
    ...
    final String policiesPath =
        String.format("/%s/%s/%s", "admin", POLICIES, namespaceName.toString());
    try {
      pulsarResources
          .getNamespaceResources()
          .set(
      ...
      log.warn(
          "[{}] Failed to set permissions for namespace {}: does not exist", role, namespaceName);
      result.completeExceptionally(
          new IllegalArgumentException("Namespace does not exist" + namespaceName));
  ...
  public CompletableFuture<Void> revokeSubscriptionPermissionAsync(
    ...
    return updateSubscriptionPermissionAsync(
        namespace, subscriptionName, Collections.singleton(role), true);
  }
  ...
  private CompletableFuture<Void> updateSubscriptionPermissionAsync(
    ...
    try {
      ...
      log.warn(
          ...
          namespace);
      result.completeExceptionally(
          new IllegalArgumentException("Namespace does not exist" + namespace));
    } catch (BadVersionException e) {
      log.warn(
          "[{}] Failed to set permissions for {} on namespace {}: concurrent modification",
          subscriptionName,
          roles,
          namespace);
      result.completeExceptionally(
      ...
      log.error(
          "[{}] Failed to get permissions for role {} on namespace {}",
          subscriptionName,
          roles,
          namespace,
          e);
  ...
  private CompletableFuture<Boolean> checkAuthorization(
      TopicName topicName, String role, AuthAction action) {
  ...
  private boolean checkCluster(TopicName topicName) {
    if (topicName.isGlobal() || conf.getClusterName().equals(topicName.getCluster())) {
      ...
      if (log.isDebugEnabled()) {
        log.debug(
            "Topic [{}] does not belong to local cluster [{}]", topicName, conf.getClusterName());
      }
      return false;
    }
  }
          ...
          .thenAccept(
              policies -> {
                if (policies.isEmpty()) {
                  ...
                  Map<String, Set<AuthAction>> topicRoles =
                      policies
                          .get()
                          .auth_policies
                  ...
                  if (topicRoles != null && role != null) {
                    ...
                    for (String cr : clientRoles) {
                      Set<AuthAction> topicActions = topicRoles.get(cr);
                  ...
                  if (conf.isAuthorizationAllowWildcardsMatching()) {
                    for (String cr : clientRoles) {
                      if (checkWildcardPermission(cr, action, namespaceRoles)) {
                        // The role has namespace level permission by wildcard match
                        permissionFuture.complete(true);
                        return;
                      }

                      if (topicRoles != null && checkWildcardPermission(cr, action, topicRoles)) {
                  ...
                  // If the partition number of the partitioned topic having topic level policy is
                  // updated,
  ...
  private void validatePoliciesReadOnlyAccess() {
    ...
    try {
      arePoliciesReadOnly =
          pulsarResources.getNamespaceResources().exists(POLICIES_READONLY_FLAG_PATH);
    } catch (Exception e) {
      log.warn(
          "Unable to fetch contents of [{}] from global zookeeper", POLICIES_READONLY_FLAG_PATH, e);
  ...
  public CompletableFuture<Boolean> allowNamespaceOperationAsync(
    ...
    switch (operation) {
      case PACKAGES:
        isAuthorizedFuture =
            allowTheSpecifiedActionOpsAsync(namespaceName, role, authData, AuthAction.packages);
        break;
      default:
    ...
    return isTenantAdminFuture.thenCombine(
        isAuthorizedFuture,
        (isTenantAdmin, isAuthorized) -> {
          if (log.isDebugEnabled()) {
            log.debug(
                "Verify if role {} is allowed to {} to topic {}: isTenantAdmin={}, isAuthorized={}",
                role,
                operation,
                namespaceName,
                isTenantAdmin,
  ...
  public CompletableFuture<Boolean> allowTopicOperationAsync(
      ...
      AuthenticationDataSource authData) {
    log.debug(
        "Check allowTopicOperationAsync ["
            + operation.name()
            + "] on ["
            + topicName.toString()
            + "].");
                  ...
                  TenantInfo tenantInfo =
                      pulsarResources
                          ...
                          .orElseThrow(
                              () ->
                                  new RestException(
                                      Response.Status.NOT_FOUND, "Tenant does not exist"));
  ...
  public CompletableFuture<Boolean> isSuperUser(
    ...
    return CompletableFuture.completedFuture(
        clientRoles.stream().anyMatch(superUserRoles::contains));
  }
  ...
  public CompletableFuture<Boolean> isTenantAdmin(
    ...
    var result = false;
    var adminRoles = tenantInfo.getAdminRoles();
    if (CollectionUtils.isNotEmpty(adminRoles)) {
      result = clientRoles.stream().anyMatch(adminRoles::contains);
    }
