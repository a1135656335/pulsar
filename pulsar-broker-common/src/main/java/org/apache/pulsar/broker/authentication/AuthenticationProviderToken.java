...
public class AuthenticationProviderToken implements AuthenticationProvider {
  ...
  @Override
  public AuthenticationState newAuthState(AuthData authData, SocketAddress remoteAddress,
    ...
    try {
      ...
      if (audienceClaim != null) {
        ...
        if (object == null) {
          throw new JwtException(
              "Found null Audience in token, for claimed field: " + audienceClaim);
        }
      ...
      if (jwt.getBody().getExpiration() != null) {
        expiringTokenMinutesMetrics.observe(
            (double) (jwt.getBody().getExpiration().getTime() - new Date().getTime()) / (60
                * 1000));
      }
      return jwt;
  ...
  private SignatureAlgorithm getPublicKeyAlgType(ServiceConfiguration conf)
  ...
  private static final class TokenAuthenticationState implements AuthenticationState {
    ...
    public AuthData authenticate(AuthData authData) throws AuthenticationException {
      ...
      this.authenticationDataSource = new AuthenticationDataCommand(token, remoteAddress,
          sslSession);
      if (jwt.getBody().getExpiration() != null) {
